...
trend_pvals <- emtrends(log_food_kcal_tp, ~ meal_type, var = "avocado_intake_num") %>% 
  summary(infer = TRUE, adjust = "bonferroni") %>% 
  as.data.frame() %>%
  select(meal_type, p.value) %>%
  rename(trend_pval  = p.value) %>%
  mutate(trend_pval = format.pval(trend_pval, digits = 2, eps = 0.0001) %>%
           str_replace("1e-04", ".0001"))
...
emmeans_tab %>%
  ...
  left_join(trend_pvals, by = "meal_type") %>%
  mutate(
    trend_pval  = ifelse(avocado_intake_cat == "No avocado at all", trend_pval, "")
  ) %>%
  kable(
    col.names = c(
      "Meal type",
...
trend_pvals <- emtrends(energy_density_tp, ~ meal_type, var = "avocado_intake_num") %>% 
  summary(infer = TRUE, adjust = "bonferroni") %>% 
  as.data.frame() %>%
  select(meal_type, p.value) %>%
  rename(trend_pval  = p.value) %>%
...
trend_pvals <- emtrends(log_food_kcal_tp, ~ meal_type, var = "lag_avocado_intake_num") %>% 
  summary(infer = TRUE, adjust = "bonferroni") %>% 
  as.data.frame() %>%
  select(meal_type, p.value) %>%
  rename(trend_pval  = p.value) %>%
...
emmeans_tab %>%
  ...
  left_join(trend_pvals, by = "meal_type") %>%
  mutate(
    trend_pval  = ifelse(lag_avocado_intake_cat == "No avocado at all", trend_pval, "")
  ) %>%
  kable(
    col.names = c(
      "Meal type",
...
emmeans_tab %>% 
  ...
  facet_wrap(~ meal_or_snack, scales = "free_x") +
  labs(
    title = "Estimated marginal means (with 95% CI) of energy content",
    x = "", 
    y = "Adjusted mean energy content (kcal/gram)",
    fill = "Preceding EO") +
  theme(legend.position = "bottom")
...
trend_pvals <- emtrends(meal_interval_tp, ~ meal_type, var = "lag_avocado_intake_num") %>% 
  summary(infer = TRUE) %>% 
  as.data.frame() %>%
  select(meal_type, p.value) %>%
  rename(trend_pval  = p.value) %>%
  mutate(trend_pval = format.pval(trend_pval, digits = 4, eps = 0.0001) %>%
           str_replace("1e-04", ".0001") %>% 
           str_replace("1.00000", ".9999"))
